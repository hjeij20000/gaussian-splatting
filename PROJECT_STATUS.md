...
**3DGUT local status:** gsplat upgraded to 1.5.3 on main branch ✅. COLMAP parser patched for modern pycolmap 3.13+ ✅. Trainer deps installed in venv ✅. GPU required to run (local GPU currently in power-error state — needs reboot). Dockerfile updated with all 3DGUT deps for v12 build.
